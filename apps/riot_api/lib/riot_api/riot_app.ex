defmodule RiotApi.RiotApp do
  use GenServer

  @spec start_link(RiotApi.RiotClient.riot_api_token(), (RiotApi.Region.t() | RiotApi.Platform.t()), Tesla.Client.adapter(), GenServer.options()) :: GenServer.on_start()
  ...
  @type init_args :: {RiotApi.RiotClient.riot_api_token(), (RiotApi.Region.t() | RiotApi.Platform.t()), Tesla.Client.adapter()}

  @impl true
  @spec init(init_args()) :: {:ok, state()}
  def init({api_token, region_or_platform, adapter}) do
    client = RiotApi.RiotClient.new(api_token, region_or_platform, adapter)
    {:ok, {[], client, region_or_platform}}
  end

  @type state :: {[RiotApi.GreedyRatelimiter], Tesla.Env.client(), (Region.t() | Platform.t())}
  ...
  def handle_call({:request, url, query}, _from, {ratelimiters, client, region_or_platform}) do
    ...
    :telemetry.span([:riot_api, :request, :app], metadata, fn ->
      case RiotApi.GreedyRatelimiter.reserve_all(ratelimiters) do
        {:ok, new_ratelimiters} ->
          case Tesla.get(client, url, query: query) do
            {:ok, response} ->
              case response.status do
                x when x >= 200 and x < 300 ->
                  {app, method_limit_info} = RiotApi.RiotRateLimitInfo.from_http_headers(Map.new(response.headers))
                ...
                429 ->
                  ...
                  {{:reply, {:error, backoff}, {[RiotApi.GreedyRatelimiter.new(1, 1, backoff, DateTime.now!("Etc/UTC"))], client, region_or_platform}}, metadata}
                other ->
                  other |> IO.inspect()
  ...
  def apply_infos(ratelimiters, infos) do
    ...
    |> Enum.map(fn %{count: count, limit: limit, interval_seconds: seconds} ->
      ...
      case found do
        nil -> RiotApi.GreedyRatelimiter.new(count, limit, seconds, DateTime.now!("Etc/UTC"))
        found -> %RiotApi.GreedyRatelimiter{found | count: count}
      end
    end)
  end
end
